...
//TODO: load images - wait for loading the image
...
export class Clown{
    ...
    private drawHead(pose: Pose){
        ...
        // distance in the actual image
        const eyeWidth = 205/1.2
        const eyesToMouth = 220/1.2

        // const eyeWidth = 205*2.5
        // const eyesToMouth = 150*2.5

        if( !leftEye || !rightEye || !leftMouth || !rightMouth ) {
            return 
        }

        const eyeAngle = getAngle(leftEye, rightEye)
        ...
        //console.log('mouthToEyeDistance', mouthToEyeDistance, ' middleMouth', middleMouth)
    ...
    private drawImage(options:{
        ...
        const{
            ...
            this.rendering_context.save()

            this.rendering_context.translate(x, y)  

            this.rendering_context.rotate(((180+rotation)*Math.PI)/180)

            this.rendering_context.drawImage(image, 0-(width*offsetX), 0-(height*offsetY), width, height )

            this.rendering_context.restore()
